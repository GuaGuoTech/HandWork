        ...
        public  static List<JobList>  GetAllRecruitByType(int pageNum, int maxList, int type,Expression<Func<Recruit,bool>> ex=null)
        {
            using (EntityContext entityContext = new EntityContext())
            {
              ...
              else
              {
                  ...
                  if (type == 4)
                  {
                      recruitList = entityContext.Recruits.Where(r=>r.state==1).ToList().OrderByDescending(r => r.PublishTime).Skip(num).Take(maxList).ToList();
                  }
                  else
                  {
                      recruitList = entityContext.Recruits.Where(r => r.PayType == type&&r.state==1).OrderByDescending(r => r.PublishTime).Skip(num).Take(maxList).ToList();
                  }
              }
